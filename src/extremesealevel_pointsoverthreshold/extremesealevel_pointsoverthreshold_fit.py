...
def extremesl_fit(station_data: dict, pipeline_id: str) -> dict:
    ...
    for station_id in station_data:
        ...
        gp_params = genpareto.fit(
            extremes_loc["height"].values - loc, loc=0, scale=sigma0
        )

        # calculate covariance matrix of estimated parameters
        gp_nlogl, gp_cov = gplike(
            gp_params[0], gp_params[2], extremes_loc["height"].values - loc
        )
        ...
        fitted_data[station_id] = {
            "loc": loc,
            "gp_shape": gp_params[0],
            "gp_scale": gp_params[2],
            "gp_cov": gp_cov,
            "extremes_loc": extremes_loc,
            "avg_exceed": avg_exceed,
            "decl_esls_pyear": decl_esls_pyear,
            "esl_years": esl_years,
            "mhhw": mhhw,
            "mhhwFreq": mhhwFreq,
            "obs": obs,
